...
async def send_media_group_after_delay(media_group_id, user_id, state):
    ...
    async with media_groups_lock:
        ...
        if media_group_id in media_groups:
            ...
            logging.info(f"User {user_id} created new mediagroup-question in channel: {MAIN_CHANNEL_ID}")
...
async def process_photo(message: types.Message, state: FSMContext):
    ...
    message_from_chat = await bot.send_photo(MAIN_CHANNEL_ID, photo_id, caption = await default_post_text(await get_username(user_id), user_login, caption), parse_mode="HTML") # Отправка сообщения с обработчиком default_post_text

    save_post(user_id, f"Фото: {caption}", message_from_chat.message_id)

    logging.info(f"User {user_id} created new photo-question in channel: {MAIN_CHANNEL_ID}")
...
async def process_video(message: types.Message, state: FSMContext):
    ...
    message_from_chat = await bot.send_video(MAIN_CHANNEL_ID, video_id, caption = await default_post_text(await get_username(user_id), user_login, caption), parse_mode="HTML")

    save_post(user_id, f"Видео: {caption}", message_from_chat.message_id)

    logging.info(f"User {user_id} created new video-question in channel: {MAIN_CHANNEL_ID}")
...
async def process_document(message: types.Message, state: FSMContext):
    ...
    message_from_chat = await bot.send_document(MAIN_CHANNEL_ID, document_id, caption = await default_post_text(await get_username(user_id), user_login, caption), parse_mode="HTML")

    save_post(user_id, f"Документ: {caption}", message_from_chat.message_id)

    logging.info(f"User {user_id} created new document-question in channel: {MAIN_CHANNEL_ID}")
...
async def process_voice(message: types.Message, state: FSMContext):
    ...
    message_from_chat = await bot.send_voice(MAIN_CHANNEL_ID, voice_id, caption = await default_post_text(await get_username(user_id), user_login, caption), parse_mode="HTML")

    save_post(user_id, f"Голосовое сообщение: {caption}", message_from_chat.message_id)

    logging.info(f"User {user_id} created new voice-question in channel: {MAIN_CHANNEL_ID}")
...
async def process_voice(message: types.Message, state: FSMContext):
    ...
    message_from_chat = await bot.send_audio(MAIN_CHANNEL_ID, audio_id, caption = await default_post_text(await get_username(user_id), user_login, caption), parse_mode="HTML")

    save_post(user_id, f"Музыка / Аудио: {caption}", message_from_chat.message_id)

    logging.info(f"User {user_id} created new music-question in channel: {MAIN_CHANNEL_ID}")
